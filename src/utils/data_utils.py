...
def remove_duplicates(
    df: DataFrame, subset: Optional[List[str]] = None
) -> DataFrame:
...
def handle_missing_values(
    ...
    Args:
        ...
        fill_value: Value (or dict of column -> value) used by 'fill'
            strategy
    ...
    strategies = {
        ...
        "fill": lambda d: (
            d.na.fill(fill_value, subset=subset)
...
def add_audit_columns(
    df: DataFrame, created_by: str = "ml_pipeline"
) -> DataFrame:
    ...
    """
    return df.withColumn("created_at", F.current_timestamp()).withColumn(
        "created_by", F.lit(created_by)
    )
...
def summarize_missing_values(df: DataFrame) -> Dict[str, int]:
    ...
    return {
        column: int(counts_row[column])
        for column in df.columns
        if counts_row[column]
    }
