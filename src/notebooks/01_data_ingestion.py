...
missing_columns = set(contract["required_columns"].keys()) - set(
    df_raw.columns
)
...
for column, allowed_types in contract["required_columns"].items():
    ...
    if not any(actual_type.startswith(t) for t in allowed_types):
        raise ValueError(
            f"Column '{column}' expected one of {allowed_types} "
...
for column, max_ratio in contract["max_null_ratio"].items():
    if null_ratio[column] > max_ratio:
        raise ValueError(
            f"Data contract failed for '{column}': null ratio "
            f"{null_ratio[column]:.4f} exceeds {max_ratio:.4f}"
        )

duplicate_count = (
    raw_count - df_raw.select("id", "timestamp").dropDuplicates().count()
)
duplicate_ratio = float(duplicate_count) / raw_count if raw_count > 0 else 0.0
if duplicate_ratio > contract["max_duplicate_ratio"]:
    raise ValueError(
        f"Data contract failed: duplicate ratio "
        f"{duplicate_ratio:.4f} exceeds {contract['max_duplicate_ratio']:.4f}"
    )

display(
    spark.createDataFrame(
        [(k, v) for k, v in null_ratio.items()], ["column", "null_ratio"]
    )
)
...
# Incremental processing: keep only records newer than the latest
# processed timestamp.
...
if latest_processed_timestamp is not None:
    df_raw = df_raw.filter(
        F.col("timestamp") > F.lit(latest_processed_timestamp)
    )
    print(
        "Running incremental ingestion from watermark: "
...
df_transformed = (
    df_raw.filter(F.col("id").isNotNull())
...
if spark.catalog.tableExists(target_table):
    ...
    delta_target.alias("t").merge(
        df_transformed.alias("s"), "t.id = s.id AND t.timestamp = s.timestamp"
    ).whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
else:
    df_transformed.write.format("delta").partitionBy("ingestion_date").mode(
        "overwrite"
...
result = {
    ...
    "table_name": target_table,
    "mode": (
        "incremental" if latest_processed_timestamp is not None else "full"
    ),
    "quality_gates": {
