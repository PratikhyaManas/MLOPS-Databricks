...
def get_spark_session() -> SparkSession:
    ...
    return (
        session if session is not None else SparkSession.builder.getOrCreate()
    )


def log_notebook_run(
    notebook_name: str, status: str = "started", message: str = ""
) -> None:
    ...
    # Preserve shape for potential downstream structured logger integration.
    _ = log_entry
...
def add_processing_metadata(
    df: DataFrame, source: str, stage: str
) -> DataFrame:
    ...
    """
    return (
        df.withColumn("data_source", F.lit(source))
...
def profile_dataframe(df: DataFrame) -> Dict[str, Any]:
    ...
    return {
        ...
        "memory_size": (
            str(df.memory_usage(deep=True).sum())
