...
baseline_exprs = [F.avg(F.col(c).cast("double")).alias(c) for c in feature_cols if c in df_baseline.columns]
...
per_feature_drift = {}
for c in current_avg.keys():
    baseline_value = baseline_avg.get(c)
    current_value = current_avg.get(c)
    if baseline_value is None or current_value is None:
        continue
    denom = max(abs(float(baseline_value)), 1e-6)
    per_feature_drift[c] = abs(float(current_value) - float(baseline_value)) / denom

drift_score = sum(per_feature_drift.values()) / len(per_feature_drift) if per_feature_drift else 0.0
